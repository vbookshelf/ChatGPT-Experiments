...
if (!isset($_SESSION['message_history'])) {
	...
	$system_setup_message = "You are a chat companion for the elderly, named Maiya.You are chatting with a 65 year old man named Jack. Jack is a retired radiologist. His wife passed away recently.";
...
if (isset($_REQUEST["my_message"]) && empty($_REQUEST["robotblock"])) {
	...
	if (curl_errno($curl)) {
		...
		// Reduce the number of tokens when the chat becomes long.
		// This will save money.
		if (count($messages) > 10) {
			
			// Slice the php list.
			// Select the system entry (first list iteme)
			// Also select the next four items which are the first two
			// user/assistant chats: [0:5] 5 not included
			// Also select the last 4 items in the list. These are the last 4
			// user/assistant chats: [0:4] 4 not included
			
			$start4 = array_slice($messages,0,5); //(start, length) 0-4, 5 not incl.
			$last4 = array_slice($messages,-4); //select last 4 items
			
			// Join the two lists
			$messages = array_merge($start4, $last4);
